...
class ActionFrames(data.Dataset):
    def __init__(self, video_segs, transform):
        self.video_segs = video_segs
        self.transform = transform

    def get(self, index):
        print(f"processing video segment - {index}")
        images = [Image.open(fpath).convert('RGB') for fpath in self.video_segs[index]]
        process_data = self.transform(images)
        return process_data

    def __getitem__(self, index):
        print(f"get segment - {index}")
        return self.get(index)

    def __len__(self):
        return len(self.video_segs)
